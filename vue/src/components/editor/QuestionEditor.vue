 ...
 <!-- Question index -->
  <div class="flex items-center justify-between">
    ...
    <div class="flex items-center">
      ...
      <!-- Delete question -->
      <button
        type="button" @click="deleteQuestion()" class="flex items-center text-xs py-1 px-3 rounded-sm border border-transparent text-red-500 hover:border-red-600">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
...
// Cek jika pertanyaa/ Question mempunyai opsi
function shouldhaveOptions() {
  return ["select", "radio", "checkbox"].includes(model.value.type);
}
...
function typeChange() {
  if(shouldhaveOptions()) {
    setOptions(getOptions() || []); // untuk menjaga selected jika switch back data akan tetap terselect
  }
  dataChange()
}
